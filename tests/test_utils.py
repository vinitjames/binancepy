...
class TestUtils(unittest.TestCase):
    ...
    def test_interval_to_ms(self):
        ...
        self.assertEqual(interval_to_ms(
            kline_interval.THREEMINUTE), 3 * 60 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.FIVEMINUTE), 5 * 60 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.FIFTEENMINUTE), 15 * 60 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.THIRTYMINUTE), 30 * 60 * 1000)
        self.assertEqual(interval_to_ms(kline_interval.ONEHOUR), 3600 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.TWOHOUR), 2 * 3600 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.FOURHOUR), 4 * 3600 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.SIXHOUR), 6 * 3600 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.EIGHTHOUR), 8 * 3600 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.TWELVEHOUR), 12 * 3600 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.ONEDAY), 24 * 3600 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.THREEDAY), 72 * 3600 * 1000)
        self.assertEqual(interval_to_ms(
            kline_interval.ONEWEEK), 7 * 24 * 3600 * 1000)
